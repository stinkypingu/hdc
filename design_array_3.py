...
print(pos_cooc.shape)
print(pos_cooc[:6, :6])

neg_cooc = algo.get_cooccurrence_matrix(neg_features, 1000)
print(neg_cooc.shape)
print(neg_cooc[:6, :6])
'''

pos_train_features, pos_test_features = train_test_split(pos_features, test_size=0.2, random_state=42)
neg_train_features, neg_test_features = train_test_split(neg_features, test_size=0.2, random_state=42)
...
for i in range(3):
    ...
    pos_train_encodings, pos_train_inc_matrix, pos_train_dec_matrix = encode(pos_train_features, projection_matrix)
    neg_train_encodings, neg_train_inc_matrix, neg_train_dec_matrix = encode(neg_train_features, projection_matrix)

    pos_test_encodings, pos_test_inc_matrix, pos_test_dec_matrix = encode(pos_test_features, projection_matrix)
    neg_test_encodings, neg_test_inc_matrix, neg_test_dec_matrix = encode(neg_test_features, projection_matrix)    


    pos_am, neg_am = algo.calculate_AMs(pos_train_encodings, neg_train_encodings)
    ...
    out = algo.make_predictions(pos_train_encodings, pos_am, neg_am)
    print(f'{np.sum(out) / out.shape[0] * 100 :.2f}%, {np.sum(out)}/{out.shape[0]}')

    out = algo.make_predictions(neg_train_encodings, pos_am, neg_am)
    print(f'{(out.shape[0] - np.sum(out)) / out.shape[0] * 100:.2f}%, {(out.shape[0] - np.sum(out))}/{out.shape[0]}')

    #results for test
    print("\nresults for test data:")
    out = algo.make_predictions(pos_test_encodings, pos_am, neg_am)
    print(f'{np.sum(out) / out.shape[0] * 100 :.2f}%, {np.sum(out)}/{out.shape[0]}')

    out = algo.make_predictions(neg_test_encodings, pos_am, neg_am)
    print(f'{(out.shape[0] - np.sum(out)) / out.shape[0] * 100:.2f}%, {(out.shape[0] - np.sum(out))}/{out.shape[0]}')


    #flip 
    
    pdiffy = pos_train_inc_matrix - pos_train_dec_matrix
    ndiffy = neg_train_inc_matrix - neg_train_dec_matrix
